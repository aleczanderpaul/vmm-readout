...
from scipy.stats import crystalball, cauchy, norm
...
# Fit a Lorentzian function to fe55 events
def fitLorentzian(df, plot=True, saveFig=True):
    try:
        # Get gain values
        gain = df.gain
        # Keep only gain entries with z-score < 3 (exclude outlier which may be cosmic tracks or nuclear recoils)
        ...
        # Define Range and Fit :
        try:
            coeff, covar = curve_fit(cauchy.pdf, bin_centers2[nz2], hist2[nz2], sigma=n_err2, absolute_sigma=True, p0=(mu_guess,1600)) #p0 = mu, FWHM/2
        except:
            raise Exception("Fit failed")


        f_opti = cauchy.pdf(bin_centers,*coeff)

        perr = np.sqrt(np.diag(covar))

        if np.absolute(perr[0]) > np.absolute(coeff[0]):
            raise Exception("Poor fit")
        ...
        return coeff[0], perr[0], coeff[1], perr[1], charge_sharing, mu_e_x, mu_e_y

    except:
        print("-fit failed-")
        return np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan

# Fit a Gaussian function to fe55 events
def fitGaussian(df, plot=True, saveFig=True):
    try:
        # Get gain values
        gain = df.gain
        # Keep only gain entries with z-score < 3 (exclude outlier which may be cosmic tracks or nuclear recoils)
        ...
        # Define Range and Fit :
        try:
            coeff, covar = curve_fit(norm.pdf, bin_centers2[nz2], hist2[nz2], sigma=n_err2, absolute_sigma=True, p0=(mu_guess,1600)) #p0 = mu, sigma
        except:
            raise Exception("Fit failed")


        f_opti = norm.pdf(bin_centers,*coeff)
